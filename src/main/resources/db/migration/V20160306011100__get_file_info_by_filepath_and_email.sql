...
begin
    ...
    loop       
        if exists(select 1 from information_schema.schemata where schema_name = email_which_shares) then
            return query
               execute 'select f.file_id, f.file_uid, f.file_name, f.file_path, f.file_size, f.create_date, f.update_date, f.is_folder, f.extension, f.group_id, f.parent_file_id, u.user_id, u.user_uid, u.user_name, u.last_name from ' 
